namespace Quantum
{
    ...
    [Preserve]
    public unsafe class UnitsSystem2 : SystemMainThreadFilter<UnitsSystem2.Filter>, ISignalOnEntityPrototypeMaterialized
    {
        ...
        public override void Update(Frame f, ref Filter filter)
        {
            ...
            switch (unit->state) 
            {
                ...
                case UnitState.Moving:
                    ...
                    {
                        if (FPVector3.Distance(filter.Transform->Position, pathfinder->Target) > resourceContainer.workerStopDistance) return;
                    }
        ...
        private bool TryToFindTarget(Frame f, EntityRef entity, UnitComponent* unit, Transform3D transform)
        {
                ...
                unit->inventory.Amount <= 0 && unit->state == UnitState.Idle)
            {
                ...
                f.Events.UnitMoving(entity, f.Get<Transform3D>(unit->targetEntity).Position);
                return true;
            }
            return false;
        }
        ...
        private void SetUnitState(Frame f, EntityRef unitEntity, UnitState state) 
        {
            ...
            switch (unit->state)
            {
                ...
                case UnitState.Moving:
                    ...
                    t->LookAt(dest);
                    f.Events.UnitMoving(unitEntity, dest); break;
                case UnitState.Harversting:
